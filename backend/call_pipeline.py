...
class CallSession:
    ...
    async def dispatch_order_if_ready(self, max_retries: int = 3) -> bool:
        ...
        self._order_dispatched = True
        result = await send_order_to_kitchen(self.order, self.restaurant, self.db, self.menu_index)
                    ...
                    if session.business_type == "restaurant":
                        ...
                        if not session.order.items and session.transcript and not session._escalated:
                            ...
                            if extracted:
                                ...
                                result = await send_order_to_kitchen(extracted, session.restaurant, session.db, session.menu_index)
