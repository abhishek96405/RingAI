...
| PR-K | 🔧 | Operator failure-visibility & Pro features (K1 A8-6/C12-1 per-restaurant voice ✔️ — create_call_pipeline resolves voice from config.voice_id (session.config) instead of overwriting with env GEMINI_VOICE; the VoiceAndAITab picker now drives live calls · K2a A7-17 ✔️ — Gemini-unavailable call-analysis fallback no longer fabricates a quality_score/issues/highlights; returns analysis_available=false + null score + honest summary, real path sets analysis_available=true; C16-3 ✔️ (K2b) — CallsPage shows "Unavailable" + muted note when analysis_available=false · K3a B5-38 ✔️ — removed the threshold-5 auto-apply (suggestions stay pending); built the missing approve endpoint (applies alias + marks applied + bumps learned stat) and reject endpoint (dismisses, won't resurface); get_pending_suggestions exposes a stable id · K3b C24-1 ✔️ — AILearningWidget pending aliases are now approve/reject rows (approve adds to menu, reject dismisses), and the widget moved to its own Pro-gated, restaurant-only "AI Learning" tab at /dashboard/learning. K4 B5-26/C21-1 ✔️ — delivery now fails CLOSED: validate_delivery_distance enforces the delivery_zip_codes allowlist deterministically and returns allowed/verified instead of within_radius=true-on-every-error; dispatch_order_if_ready rejects out-of-area orders (accurate customer SMS) and holds unverifiable ones for operator review (SMS + dashboard alert). · K5 C9-1 ✔️ — OrdersPage gets a page-level dispatch-failure banner + "Show failed" filter on top of the existing per-row DISPATCH_FAILED badge (persistence verified end-to-end: _apply_dispatch_result writes state+reason, to_dict serializes them as a str-enum); unverifiable delivery holds now transition to DISPATCH_FAILED so they surface there too. Remaining PR-K: K6 C9-3, K7 C21-4, K8 C22-1) | C9-1, C9-3, C16-3, A7-17, C24-1/B5-38, C12-1/A8-6, C21-4, C21-1/B5-26, C22-1 |
