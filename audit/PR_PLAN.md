...
| PR-K | 🔧 | Operator failure-visibility & Pro features (K1 A8-6/C12-1 per-restaurant voice ✔️ — create_call_pipeline resolves voice from config.voice_id (session.config) instead of overwriting with env GEMINI_VOICE; the VoiceAndAITab picker now drives live calls · K2a A7-17 ✔️ — Gemini-unavailable call-analysis fallback no longer fabricates a quality_score/issues/highlights; returns analysis_available=false + null score + honest summary, real path sets analysis_available=true; C16-3 ✔️ (K2b) — CallsPage shows "Unavailable" + muted note when analysis_available=false · K3a B5-38 ✔️ — removed the threshold-5 auto-apply (suggestions stay pending); built the missing approve endpoint (applies alias + marks applied + bumps learned stat) and reject endpoint (dismisses, won't resurface); get_pending_suggestions exposes a stable id; C24-1 UI next (K3b)) | C9-1, C9-3, C16-3, A7-17, C24-1/B5-38, C12-1/A8-6, C21-4, C21-1/B5-26, C22-1 |
