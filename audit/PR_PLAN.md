...
**Updated:** 2026-06-18 · `ringai-deploy` @ `d0b2995b` — PR-L ✔️ (landing redesign + fabricated-claims/rebrand cleanup); PR-M ✔️ (Privacy + Terms pages live, footer wired — Stripe-live legal gate cleared)
**Updated:** 2026-06-18 · `ringai-deploy` @ `e45d10fd` — PR-K ✔️ (verified in code): K6/C9-3 OrdersPage pagination, K7/C21-4 atomic fulfillment save, K8/C22-1 owner-rule sanitization + safety precedence
...
| PR-K | ✔️ | Operator failure-visibility & Pro features (K1 A8-6/C12-1 per-restaurant voice ✔️ — create_call_pipeline resolves voice from config.voice_id (session.config) instead of overwriting with env GEMINI_VOICE; the VoiceAndAITab picker now drives live calls · K2a A7-17 ✔️ — Gemini-unavailable call-analysis fallback no longer fabricates a quality_score/issues/highlights; returns analysis_available=false + null score + honest summary, real path sets analysis_available=true; C16-3 ✔️ (K2b) — CallsPage shows "Unavailable" + muted note when analysis_available=false · K3a B5-38 ✔️ — removed the threshold-5 auto-apply (suggestions stay pending); built the missing approve endpoint (applies alias + marks applied + bumps learned stat) and reject endpoint (dismisses, won't resurface); get_pending_suggestions exposes a stable id · K3b C24-1 ✔️ — AILearningWidget pending aliases are now approve/reject rows (approve adds to menu, reject dismisses), and the widget moved to its own Pro-gated, restaurant-only "AI Learning" tab at /dashboard/learning. K4 B5-26/C21-1 ✔️ — delivery now fails CLOSED: validate_delivery_distance enforces the delivery_zip_codes allowlist deterministically and returns allowed/verified instead of within_radius=true-on-every-error; dispatch_order_if_ready rejects out-of-area orders (accurate customer SMS) and holds unverifiable ones for operator review (SMS + dashboard alert). · K5 C9-1 ✔️ — OrdersPage gets a page-level dispatch-failure banner + "Show failed" filter on top of the existing per-row DISPATCH_FAILED badge (persistence verified end-to-end: _apply_dispatch_result writes state+reason, to_dict serializes them as a str-enum); unverifiable delivery holds now transition to DISPATCH_FAILED so they surface there too. K6 C9-3 ✔️ (OrdersPage loads all order-bearing calls to a 50-page/5k cap + "Showing N of M" paging), K7 C21-4 ✔️ (single atomic PUT /api/restaurants/{id}/fulfillment — Mongo txn writes restaurant+config or neither; test_api_fulfillment), K8 C22-1 ✔️ (owner rules sanitized via _sanitize_rule + subordinated to the safety/allergen block in build_system_prompt; test_gemini_service_prompt_assembly) — PR-K COMPLETE) | C9-1, C9-3, C16-3, A7-17, C24-1/B5-38, C12-1/A8-6, C21-4, C21-1/B5-26, C22-1 |
| PR-L | ✔️ | Marketing-claims & rebranding cleanup (LAUNCH-GATING: FTC/legal) — landing rebuilt with real content; SOC-2/testimonials/stats/fake-integration claims removed, multilingual reframed "coming soon", RingAI→Duuutah rebrand complete | C32-1, C28-1, C31-1, C30-3, C29-1/C34-1, C33-1, C27-1, C12-2, A2-6, A1-6, A7-4 |
| PR-M | ✔️ | Legal pages — Privacy + Terms (Security folded into Privacy; GDPR dropped — US-only) (LAUNCH-GATING: Stripe live) — both pages live + footer wired | C35-1 |
