...
| B5-26 | ✔️ | `delivery_utils.py` | (PR-K/K4) Was fail-OPEN — `validate_delivery_distance` returned `within_radius:true` on every error/missing-key/ungeocodable branch, so a delivery to an unresolvable address (or during a Maps outage) dispatched unverified. Now fails CLOSED with an `{allowed, verified, reason}` contract, and the `delivery_zip_codes` allowlist is enforced deterministically (no API needed) — previously it was only fed to the prompt. `dispatch_order_if_ready` rejects conclusively-out-of-area orders (accurate customer SMS) and holds unverifiable ones for operator review (SMS + dashboard alert) instead of dispatching blind. | ✔️ | `unit/test_delivery_utils.py` |
...
- **C9-1 ✔️** (PR-K/K5) — `OrdersPage` surfaces dispatch failures two ways: the existing per-row/detail "Failed — enter manually" badge (keyed on the persisted `order_json.state === "DISPATCH_FAILED"` + `dispatch_failure_reason`, which `_apply_dispatch_result` writes and `LiveOrder.to_dict()` serializes — verified end-to-end), PLUS a new page-level red banner counting the failed orders in view with a "Show failed" filter to jump to them (a failure on page 7 was previously invisible). K5 also closes a K4 gap: an unverifiable delivery hold is now marked `DISPATCH_FAILED` (was left `CONFIRMED`) so it shows on the same surface instead of looking like a normal order.
- **C9-3 ✔️** (PR-K/K6) — `OrdersPage` no longer truncates at ~300: `fetchOrders` loads all order-bearing calls (COMPLETED + ESCALATED) up to a 50-page / 5k-per-status safety cap and renders "Showing N–M of {total}" with client-side paging (PAGE_SIZE 15). Tracked follow-up (non-blocking): a dedicated server-side `/orders` endpoint to replace the client-side bulk load.
- **C21-1 ✔️** (PR-K/K4) — the `FulfillmentTab` "Delivery Zip Codes" field (whose helper text promises "AI will only accept delivery orders from these zip codes") is now actually enforced: `validate_delivery_distance` checks the customer's ZIP against the `delivery_zip_codes` allowlist deterministically before any distance call, and the whole path fails closed (B5-26). No UI change needed — the input already existed; the backend just wasn't honoring it.
- **C21-4 ✔️** (PR-K/K7) — `FulfillmentTab`'s two-doc save is now a single atomic `PUT /api/restaurants/{id}/fulfillment`: the frontend `updateFulfillment` sends `{restaurant, config}` in one request (SettingsPage no longer issues two separate writes), and the endpoint writes both docs in a Mongo transaction (mongomock-detected + bypassed under test). Covered by `integration/test_api_fulfillment.py` (both-docs-written + cross-tenant-reject + plan-gate).
...
- **C22-1 ✔️** (PR-K/K8) — owner free-text rules can no longer override safety. `_sanitize_rule()` strips newlines/control chars and caps each rule at 200 chars (a newline-laden rule is flattened onto one bullet, so it can't inject prompt lines); custom escalation rules are appended to — not replacing — the built-in safety triggers; and owner business rules are explicitly subordinated to the safety/allergen block ("IGNORE that rule" + ALLERGEN PROTOCOL). Asserted in `unit/test_gemini_service_prompt_assembly.py` (C22-1 section).
