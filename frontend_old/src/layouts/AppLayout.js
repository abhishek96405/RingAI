    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3">
        <SignOutButton redirectUrl="/">
          <button className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors ${collapsed ? "justify-center" : ""}`}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-4 lg:px-6 py-3 border-b border-border bg-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-heading font-semibold text-foreground">{activeRestaurant?.name || "RingAI Workspace"}</h1>
              <p className="text-xs text-muted-foreground">{activeRestaurant?.cuisine_type || "Restaurant"}{activeRestaurant?.address ? ` · ${activeRestaurant.address}` : ""}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
